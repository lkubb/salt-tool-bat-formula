...
We will see a quick setup to configure the ``tool_bat`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_bat`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_bat`` formulas under ``/srv/salt/tool_bat/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_bat/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_bat/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_bat/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_bat/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_bat`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_bat`` formulas under ``/srv/salt/tool_bat/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_bat/parameters/roles
...
We create a configuration for the role ``tool_bat/server`` in ``/srv/salt/tool_bat/parameters/roles/tool_bat/server.yaml``:
...
We create another configuration for the role ``tool_bat/client`` in ``/srv/salt/tool_bat/parameters/roles/tool_bat/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_bat/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_bat/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_bat/parameters/dns:domain/  '/srv/salt/tool_bat/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-bat-as-json:
      test.nop:
        - name: {{ bat | json }}
...
.. code-block:: sls
    ...
    bat-config-file-file-managed:
      file.managed:
        - name: {{ bat.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='bat-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``bat`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ bat['winner'] }}
